...
// Language loader functions - using inline translations
const languageLoaders: Record<string, () => Promise<{ default: TranslationData }>> = {
  ...
  'zh-TW': () =>
    Promise.resolve({
      default: (allTranslations['zh-TW'] || allTranslations.zh) as TranslationData,
    }),
...
export async function loadTranslations(language: string): Promise<TranslationData> {
  ...
  if (translationCache[language]) {
    return translationCache[language] as TranslationData;
  }
 ...
 */
export function getTranslationValue(translations: TranslationData, path: string): string {
 ...
 */
export function getTranslationWithFallback(path: string, language: string): string {
...
export async function preloadLanguages(languages: string[]): Promise<void> {
  await Promise.all(languages.map((lang) => loadTranslations(lang).catch(() => {})));
}
