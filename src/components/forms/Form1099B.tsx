...
const Form1099B: React.FC<Form1099BProps> = ({ onComplete, defaultValues = {}, allowMultiple = true }) => {
  ...
  const handleComplete = () => {
    ...
    // 分类汇总
    const summary = finalTransactions.reduce((acc, transaction) => {
  ...
  // 计算当前汇总
  const currentSummary = transactions.reduce((acc, transaction) => {
    const gainLoss = parseFloat(transaction.gainLoss) || 0;
    const isLongTerm = transaction.shortLong === 'long';
  ...
  const transactionColumns = [
    ...
    },
    {
      ...
      render: (value, record) => {
        ...
        return (
          <div>
            <Text style={{ color: isProfit ? '#52c41a' : '#f5222d' }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            >
              <p className="ant-upload-drag-icon">
                {uploading ? <CalculatorOutlined style={{ fontSize: '48px' }} /> : <CameraOutlined style={{ fontSize: '48px', color: '#1890ff' }} />}
              </p>
          ...
          <div style={{ marginTop: '24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div>
                  <Text>短期盈亏: </Text>
                  <Text style={{ color: currentSummary.shortTerm >= 0 ? '#52c41a' : '#f5222d', fontWeight: 'bold' }}>
                    {formatCurrency(currentSummary.shortTerm)}
                  </Text>
                </div>
                <div>
                  <Text>长期盈亏: </Text>
                  <Text style={{ color: currentSummary.longTerm >= 0 ? '#52c41a' : '#f5222d', fontWeight: 'bold' }}>
