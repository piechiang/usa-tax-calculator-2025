...
interface TaxSavingsOpportunitiesProps {
  taxResult: TaxResult;
  inputData: InputData;
  onApplyStrategy?: (opportunity: Opportunity) => void;
}

const TaxSavingsOpportunities: React.FC<TaxSavingsOpportunitiesProps> = ({ taxResult, inputData, onApplyStrategy }) => {
  const [expandedCards, setExpandedCards] = useState<string[]>([]);

  // 计算各种节税机会
  const calculateOpportunities = (): Opportunity[] => {
    const opportunities: Opportunity[] = [];
    const { adjustedGrossIncome, taxableIncome: _taxableIncome, standardDeduction = 0, itemizedDeduction = 0, marginalTaxRate = 0.22 } = taxResult;

    // 1. 传统IRA供款机会
    const currentIraContribution = inputData.adjustments?.iraDeduction || 0;
    const maxIraContribution = (inputData.taxpayer?.age || 0) >= 50 ? 8000 : 7000; // 2025年限额
    ...
    if (iraOpportunity > 0 && adjustedGrossIncome > 0) {
      ...
      opportunities.push({
        ...
        details: [
          `2025年最高可供款 $${maxIraContribution.toLocaleString()}${(inputData.taxpayer?.age || 0) >= 50 ? '（50岁以上追加$1,000）' : ''}`,
