...
exports.calculateNonRefundableCredits = exports.calculateRefundableCredits = exports.calculateOtherCredits = exports.calculatePremiumTaxCredit = exports.calculateChildAndDependentCareCredit = exports.calculateEducationCredits = exports.calculateEarnedIncomeCredit = exports.calculateOtherDependentCredit = exports.calculateChildTaxCredit = void 0;
 ...
 */
function calculateChildTaxCredit(input, taxBeforeCredits) {
    // Count qualifying children for CTC
    const qualifyingChildren = input.dependents.filter(dep => dep.isQualifyingChild &&
        dep.age < 17 &&
        dep.hasSSN &&
    ...
    const maxPerChild = input.options?.ctcMaxPerChild || constants2025_1.IRS_CONSTANTS_2025.ctc.maxPerChild;
    const maxCredit = qualifyingChildren * maxPerChild;
    // Calculate phase-out based on AGI
    const agi = input.adjustedGrossIncome || 0;
    const phaseoutThreshold = constants2025_1.IRS_CONSTANTS_2025.ctc.phaseOutThreshold[input.filingStatus];
    ...
    if (remainingCredit > 0 && qualifyingChildren > 0) {
        ...
        if (earnedIncome > 2500) {
            ...
            const maxACTC = qualifyingChildren * constants2025_1.IRS_CONSTANTS_2025.ctc.additionalChildCredit;
...
exports.calculateChildTaxCredit = calculateChildTaxCredit;
/**
 * Calculate Other Dependent Credit (ODC)
 * $500 credit for qualifying dependents who are not qualifying children for CTC
 */
function calculateOtherDependentCredit(input, taxBeforeCredits) {
    const otherDependents = input.dependents.filter(dep => !dep.isQualifyingChild ||
        dep.age >= 17 ||
        !dep.hasSSN).length;
...
exports.calculateOtherDependentCredit = calculateOtherDependentCredit;
/**
...
function calculateEarnedIncomeCredit(input) {
    ...
    const qualifyingChildren = Math.min(3, input.dependents.filter(dep => dep.isQualifyingChild &&
        (dep.age < 19 || // Under 19
    ...
    if (investmentIncome > constants2025_1.IRS_CONSTANTS_2025.eitc.investmentIncomeLimit) {
    ...
    const maxCredit = constants2025_1.IRS_CONSTANTS_2025.eitc.maxCredits[qualifyingChildren];
    const phaseInRate = constants2025_1.IRS_CONSTANTS_2025.eitc.phaseInRates[qualifyingChildren];
    const phaseOutRate = constants2025_1.IRS_CONSTANTS_2025.eitc.phaseOutRates[qualifyingChildren];
    const phaseOutThreshold = constants2025_1.IRS_CONSTANTS_2025.eitc.phaseOutThresholds[input.filingStatus][qualifyingChildren];
    let eitc = 0;
...
exports.calculateEducationCredits = calculateEducationCredits;
/**
 * Calculate Child and Dependent Care Credit (Form 2441)
 */
function calculateChildAndDependentCareCredit(input, careExpenses = 0) {
    if (careExpenses <= 0) {
        return 0;
    }
    // Count qualifying individuals
    const qualifyingPersons = input.dependents.filter(dep => dep.age < 13 || // Child under 13
...
exports.calculateChildAndDependentCareCredit = calculateChildAndDependentCareCredit;
/**
 * Calculate Premium Tax Credit (Form 8962)
 * For health insurance marketplace coverage
 */
function calculatePremiumTaxCredit(input, premiumCost = 0, advancePTC = 0) {
    if (premiumCost <= 0) {
...
exports.calculatePremiumTaxCredit = calculatePremiumTaxCredit;
/**
 * Calculate other miscellaneous credits
 */
function calculateOtherCredits(input) {
...
exports.calculateOtherCredits = calculateOtherCredits;
/**
 * Helper function to calculate earned income for EITC and ACTC
 */
function calculateEarnedIncome(input) {
    // Wages from W-2
 ...
 */
function calculateInvestmentIncome(input) {
 ...
 */
function calculateRefundableCredits(input) {
...
exports.calculateRefundableCredits = calculateRefundableCredits;
/**
 * Calculate total non-refundable credits
 */
function calculateNonRefundableCredits(input, taxBeforeCredits) {
    ...
    if (input.credits?.childCareCredit) {
        const careCredit = calculateChildAndDependentCareCredit(input, input.credits.childCareCredit.expenses);
...
exports.calculateNonRefundableCredits = calculateNonRefundableCredits;
