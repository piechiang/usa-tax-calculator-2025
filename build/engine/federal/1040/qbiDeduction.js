...
exports.calculateQBILossCarryforward = exports.estimateW2Wages = exports.calculatePTPQBIDeduction = exports.calculateREITQBIDeduction = exports.aggregateQBIItems = exports.calculateQualifiedBusinessIncome = void 0;
const constants2025_1 = require("./constants2025");
/**
 * Calculate Section 199A QBI Deduction
 * Source: IRC §199A, Form 8995/8995-A
 */
function calculateQualifiedBusinessIncome(input, adjustedGrossIncome, totalDeductions) {
    ...
    const simpleThreshold = isMarriedJoint
        ? constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_THRESHOLD_MFJ
        : constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_THRESHOLD_SINGLE;
    if (taxableIncome <= simpleThreshold) {
        ...
        return calculateComplexQBIDeduction(input, totalQBI, totalWages, totalUBIA, taxableIncome, isMarriedJoint);
    }
}
exports.calculateQualifiedBusinessIncome = calculateQualifiedBusinessIncome;
/**
 * Calculate QBI deduction using simple method (Form 8995)
 * For taxpayers below the taxable income threshold
 */
function calculateSimpleQBIDeduction(totalQBI, taxableIncome) {
    // 20% of QBI or 20% of taxable income, whichever is less
    const qbiDeduction = Math.min(totalQBI * constants2025_1.CALCULATION_CONSTANTS.QBI_DEDUCTION_MAX_RATE, taxableIncome * constants2025_1.CALCULATION_CONSTANTS.QBI_DEDUCTION_MAX_RATE);
 ...
 */
function calculateComplexQBIDeduction(input, totalQBI, totalWages, totalUBIA, taxableIncome, isMarriedJoint) {
    const fullThreshold = isMarriedJoint
        ? constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_THRESHOLD_MFJ + constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_RANGE
        : constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_THRESHOLD_SINGLE + constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_RANGE;
    const simpleThreshold = isMarriedJoint
        ? constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_THRESHOLD_MFJ
        : constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_THRESHOLD_SINGLE;
    ...
    if (taxableIncome < fullThreshold) {
        ...
        phaseoutRatio = excessIncome / constants2025_1.CALCULATION_CONSTANTS.QBI_PHASEOUT_RANGE;
    }
 ...
 */
function aggregateQBIItems(businesses) {
    let totalQBI = 0;
    let totalWages = 0;
    let totalUBIA = 0;
    let hasSSTB = false;
...
exports.aggregateQBIItems = aggregateQBIItems;
/**
 * Calculate QBI deduction from Real Estate Investment Trusts (REITs)
 */
function calculateREITQBIDeduction(reitDividends, taxableIncome) {
    if (reitDividends <= 0) {
        return 0;
    }
    // 20% of REIT dividends, limited by 20% of taxable income
    const reitDeduction = Math.min(reitDividends * 0.20, taxableIncome * 0.20);
    return Math.round(reitDeduction);
}
exports.calculateREITQBIDeduction = calculateREITQBIDeduction;
/**
 * Calculate QBI deduction from Publicly Traded Partnership (PTP) income
 */
function calculatePTPQBIDeduction(ptpIncome, taxableIncome) {
    if (ptpIncome <= 0) {
        return 0;
    }
    // 20% of PTP income, limited by 20% of taxable income
    const ptpDeduction = Math.min(ptpIncome * 0.20, taxableIncome * 0.20);
    return Math.round(ptpDeduction);
}
exports.calculatePTPQBIDeduction = calculatePTPQBIDeduction;
/**
 * Estimate W-2 wages for QBI limitation (when not provided)
 * This is a rough estimation method
 */
function estimateW2Wages(scheduleCSales, scheduleCProfit) {
...
exports.estimateW2Wages = estimateW2Wages;
/**
 * Calculate carryforward of QBI losses
 * QBI losses from one year can be carried forward to offset future QBI
 */
function calculateQBILossCarryforward(currentYearQBI, priorYearLossCarryforward = 0) {
...
exports.calculateQBILossCarryforward = calculateQBILossCarryforward;
//# sourceMappingURL=qbiDeduction.js.map